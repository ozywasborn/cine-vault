...
export const FieldShootSummaryView: React.FC<FieldShootSummaryProps> = ({
  ...
  const { availableCategories, availableGearByCategory } = useMemo(() => {
    ...
    const filtered = availableGear.filter((item) => {
      if (!q) return true;
      return (
      ...
      {(viewMode === 'combined' || viewMode === 'cards') && (
        <div className="space-y-6">
          ...
          <div className="sticky top-16 z-30 space-y-3">
            ...
            {showStagingDrawer && (
              <div className="bg-white/98 backdrop-blur-md rounded-2xl border border-amber-300 p-3.5 sm:p-4 shadow-lg space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        data-1p-ignore="true"
                        spellCheck={false}
                        className="pl-7 pr-6 py-1 bg-slate-50 hover:bg-slate-100 focus:bg-white border border-slate-200 rounded-lg text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-amber-500 w-32 sm:w-44 transition-all"
                      />
                      {stagingSearchQuery && (
                        <button
                          type="button"
                          onClick={() => setStagingSearchQuery('')}
                ...
                {availableCategories.length > 0 && (
                  <div className="flex items-center gap-1.5 overflow-x-auto pb-1 text-xs">
                    <button
                      type="button"
                      onClick={() => setSelectedStagingCategory('all')}
